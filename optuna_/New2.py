...
class DWT_MLP_Model(nn.Module):
    def __init__(self, input_channels, seq_length, pred_length ,patch_len, mlp_hidden_size, output_channels, stride = 4, padding_patch = True,  decompose_layers=3, wave='haar', 
        ...
        self.transformer = TSTiEncoder(input_channels, patch_num = patch_num, patch_len = patch_len, max_seq_len = 5000, n_layers = num_encoder_layers, d_model = d_model, n_heads = self.n_head, decompose_layer = decompose_layer)
        ...
        self.head_low = Flatten_Head(self.individual, self.n_vars, self.head_nf, self.low_len//decompose_layer,  head_dropout=self.dropout)
