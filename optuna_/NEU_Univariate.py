...
for i in indices:
    ...
    for epoch in range(num_epochs):
        ...
        for seq_x, seq_y, seq_x_mark, seq_y_mark in train_loader:
            ...
            for i in range(num_features):
                # Slice out the i-th feature across all examples in the batch
                inputs = seq_x[:, :, i:i+1]  # Shape: [batch, seq_len, 1]
    ...
    with torch.no_grad():
        for seq_x, seq_y, seq_x_mark, seq_y_mark in test_loader:
            ...
            # Concatenate the outputs along the feature dimension
            concat_outputs = torch.cat(all_outputs, dim=-1)  # Shape should match [batch, seq_len, num_features]
