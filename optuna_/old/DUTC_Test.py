...
train_dataset = Dataset_Custom(root_path=root_path, flag='train', size=size, data_path=data_path, step_size = 4)
val_dataset = Dataset_Custom(root_path=root_path, flag='val', size=size, data_path=data_path,step_size = 4)
test_dataset = Dataset_Custom(root_path=root_path, flag='test', size=size, data_path=data_path,step_size = 4)
...
class DilatedTCNBlock2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, dilation=2, dropout_rate = 0.2):
        ...
        # Determine output channels for each activation
        tanh_out_channels = out_channels // 2 + out_channels % 2  # Tanh gets one more channel if out_channels is odd
        sigmoid_out_channels = out_channels // 2
        self.conv_skip = nn.Conv1d(in_channels, out_channels, kernel_size=1)  # 1x1 conv for skip connection
        
        # Convolution layers
        self.conv_tanh = nn.Conv1d(in_channels, tanh_out_channels, kernel_size, padding=padding, dilation=dilation)
        self.conv_sigmoid = nn.Conv1d(in_channels, sigmoid_out_channels, kernel_size, padding=padding, dilation=dilation)
        
        # Instance normalization layers
        self.norm_tanh = nn.InstanceNorm1d(tanh_out_channels)
        self.norm_sigmoid = nn.InstanceNorm1d(sigmoid_out_channels)
    ...
    def forward(self, x):
        ...
        # Path for Tanh
        tanh_conv_output = self.conv_tanh(x)
        tanh_norm_output = self.norm_tanh(tanh_conv_output)
        tanh_output = torch.tanh(tanh_norm_output)
        #tanh_output = self.dropout(tanh_output)  # Apply dropout
        
        # Path for Sigmoid
        sigmoid_conv_output = self.conv_sigmoid(x)
        sigmoid_norm_output = self.norm_sigmoid(sigmoid_conv_output)
        sigmoid_output = torch.sigmoid(sigmoid_norm_output)
        #sigmoid_output = self.dropout(sigmoid_output)  # Apply dropout
        
        # Pad Sigmoid output if necessary to match Tanh output dimensions
        if self.out_channels % 2 != 0:
            # Assuming padding with zeros. Adjust if different padding behavior is desired
            zero_padding = torch.zeros_like(sigmoid_output, device=x.device)[:, :1, :]  # Create padding for one channel
            sigmoid_output = torch.cat((sigmoid_output, zero_padding), dim=1)
        
        # Concatenate along the channel dimension
        concatenated_output = torch.cat((tanh_output, sigmoid_output), dim=1)[:,:-1,:] 
        output = self.relu(concatenated_output)
...
class DilatedInception1D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_set=[3, 5, 7, 9], dilation=2):
        super(DilatedInception1D, self).__init__()
        self._time_conv = nn.ModuleList()
        self._kernel_set = kernel_set
        
        total_kernels = len(self._kernel_set)
        base_out_channels_per_kernel = out_channels // total_kernels
        extra_out_channels = out_channels % total_kernels

        for i, kern in enumerate(self._kernel_set):
            # Adjust the number of output channels for the first few kernels to account for any remainder
            if i < extra_out_channels:
                num_output_channels_per_kernel = base_out_channels_per_kernel + 1
            else:
                num_output_channels_per_kernel = base_out_channels_per_kernel
            
            # Padding is chosen to maintain sequence length considering dilation
            padding = (kern - 1) * dilation // 2
            self._time_conv.append(
                nn.Conv1d(in_channels, num_output_channels_per_kernel, kern, padding=padding, dilation=dilation)
            )
        self._reset_parameters()
        
    def _reset_parameters(self):
        for module in self._time_conv:
    ...
    def forward(self, X_in):
        # Expecting X_in to be of shape [batch size, seq_time_len, num_features]
        # Adjusting input to match Conv1d expectation: [batch, channels, seq_length]
        #X_in = X_in.permute(0, 2, 1)
        
        X_out = []
        for conv in self._time_conv:
            X_out.append(conv(X_in))
        
        # Concatenating along the channel dimension
        X_out = torch.cat(X_out, 1)
        
        # Reverting shape to [batch size, seq_time_len, num_output_channels]
...
class DWT_MLP_Model(nn.Module):
    def __init__(self, input_channels, seq_length, mlp_hidden_size, output_channels, decompose_layers=3, wave='haar', mode='symmetric', nhead=8, d_model=None, num_encoder_layers=3, dropout=0.1, dilation = 2, kernel_size = 3, attention_type = 'original', TCN_type = 'dilated', dropout_ = True, skip_ = True):
        ...
        if self.TCN_type == 'dilated':
            ...
            kernel_set = [self.kernel_size - 2, self.kernel_size, self.kernel_size + 2]  # Example way to create a set from a single size
            self.tcn_low = DilatedInception1D(input_channels, output_channels, kernel_set, self.dilation)
            self.tcn_high_list = nn.ModuleList([DilatedInception1D(input_channels, output_channels, kernel_set, self.dilation) for _ in range(decompose_layers)])
        ...
        if self.attention_type == 'original':
            self.attention = attention # log_sparse_attention  # Assuming attention is defined elsewhere
        else:
    ...
    def forward(self, x):
        ...
        if self.TCN_type == 'dilated':
            ...
            x_low_combined = x_low + x_low_combined.permute(0,2,1)
        ...
        for i, x_high in enumerate(x_highs):
            ...
            if self.TCN_type == 'dilated':
                ...
                x_high_combined = x_high + x_high_combined.permute(0,2,1)
...
TCN_types = [  'dilated']
attention_types = [ 'log']
dropout_types = [True, False]
skip_types = [True, False]
    ...
    for attention_type in attention_types:
        for dropout_enabled in dropout_types:
            for skip_enabled in skip_types:
                print(f"Running experiment with TCN_type={TCN_type}, attention_type={attention_type}, dropout_enabled={dropout_enabled}, skip_enabled={skip_enabled}")
                print('Model Running')
                dropout_rate = 0.10146011891748014 if dropout_enabled else 0.0
                # Adjust the model instantiation to include skip connection and dropout rate configurations
                model = DWT_MLP_Model(input_channels=321+4, seq_length=96, mlp_hidden_size=64, 
                                      output_channels=321+4, decompose_layers=3, 
                                      dropout=dropout_rate, dilation=2, 
                                      mode='reflect', wave='haar', kernel_size=5, 
                                      attention_type=attention_type, TCN_type=TCN_type, 
                                      num_encoder_layers=3, nhead=4, 
                                      dropout_ = dropout_enabled,
                                      skip_ = skip_enabled)

                # Define criterion and optimizer
                criterion = nn.MSELoss()
                optimizer = optim.Adam(model.parameters(), lr=0.012230056036904522, 
                                       weight_decay=1.0059977697794999e-05)
                ...
                print(f'Total Model Running Time: {total_time:.2f} seconds for configuration: TCN_type={TCN_type}, attention_type={attention_type}, dropout_enabled={dropout_enabled}, skip_enabled={skip_enabled}')
                
                # Saving training and validation losses
                with open(f'losses_{TCN_type}_{attention_type}_{dropout_enabled}_{skip_enabled}.txt', 'w') as f:
                ...
                print(f'Test Loss for configuration: TCN_type={TCN_type}, attention_type={attention_type}, dropout_enabled={dropout_enabled}, skip_enabled={skip_enabled}: {test_loss:.4f}')

                # Saving test loss
                with open(f'test_loss_{TCN_type}_{attention_type}_{dropout_enabled}_{skip_enabled}.txt', 'w') as f:
