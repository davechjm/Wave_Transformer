...
class DilatedTCNBlock(nn.Module): #Causal Dilated
    def __init__(self, in_channels, out_channels, kernel_size=3, dilation=2, dropout_rate=0.2):
...
class CausalTCNBlock(nn.Module): #Non Dilated Causal
    ...
    def forward(self, x):
        # Calculate the necessary left padding for causality
...
class DWT_MLP_Model(nn.Module):
    def __init__(self, input_channels, seq_length, mlp_hidden_size, output_channels, decompose_layers=3, wave='haar', mode='symmetric', nhead=8, d_model=None, num_encoder_layers=3, dropout=0.1, dilation = 2, kernel_size = 3):
       ...
       # self.num_decoder_layers = num_encoder_layers
        ...
        self.tcn_low = DilatedTCNBlock(input_channels, output_channels, dilation=self.dilation,kernel_size= self.kernel_size, dropout_rate= self.dropout)
        
        self.conv_high_list = nn.ModuleList([ConvBlock(input_channels, output_channels) for _ in range(decompose_layers)])
        self.tcn_high_list = nn.ModuleList([DilatedTCNBlock(input_channels, output_channels, dropout_rate= self.dropout, kernel_size= self.kernel_size, dilation=2) for _ in range(decompose_layers)])
        ...
        # Positional Encoding
        # _(self, d_input, d_model, dropout=0.1, max_len=5000, embed_type = 'fixed', freq = 'h'):
        self.pos_encoder = ProjectedPositionalEncoding(input_channels, d_model, max_len=5000)
        #self.pos_encoder = ProjectedPositionalEncoding(input_channels,d_model, dropout = self.dropout, max_len = 5000, embed_type =None, freq = 'h' )

        # Custom Transformer Encoder setup
        self.attention = log_sparse_attention  # Assuming attention is defined elsewhere
        ...
        #decoder_layers_low = [DecoderLayer(self_attention=attention, cross_attention=attention, d_model=d_model, d_ff=mlp_hidden_size, dropout=dropout, activation="relu") for _ in range(self.num_decoder_layers)]
        #self.transformer_decoder_low = Decoder(layers=decoder_layers_low, norm_layer=nn.LayerNorm(d_model))
        
        #self.transformer_decoder_high_list = nn.ModuleList(
        #    [Decoder([DecoderLayer(self_attention=attention, cross_attention=attention, d_model=d_model, d_ff=mlp_hidden_size, dropout=dropout, activation="relu") for _ in range(self.num_decoder_layers)])
        #     for _ in range(decompose_layers)]
        #)
    ...
    def forward(self, x):
        ...
        #x_low_combined = self.transformer_decoder_low(x_low_combined, cross = x_low_combined)
        x_low_combined = x_low_combined.permute(0,2,1)
        x_low_combined = x_low + x_low_combined
        ...
        for i, x_high in enumerate(x_highs):
            ...
            #x_high_combined= self.transformer_decoder_high_list[i](x_high_combined, cross = x_high_combined)
...
def objective(trial):
    ...
    mlp_hidden_size = trial.suggest_categorical('mlp_hidden_size', [64, 128,256,512,1024])
    learning_rate = trial.suggest_loguniform('learning_rate', 5e-3, 1e-1)
    weight_decay = trial.suggest_loguniform('weight_decay', 1e-5, 1e-3)
    decompose_layer = trial.suggest_int('decompose_layer', 3,5)
    dropout_rate = trial.suggest_float('dropout_rate', 0.0, 0.25)
    dilation_rate = trial.suggest_categorical('dilation_rate', [1,2,4,8])
    mode_ = trial.suggest_categorical('mode', [ 'symmetric', 'reflect'])
    wave_ = trial.suggest_categorical('wave', ['haar', 'db1'])
    shuffle_ = trial.suggest_categorical('shuffle', [True, False])
    step_size_ = trial.suggest_categorical('step_size', [1,2,4,8,16])
    kernel_size_ = trial.suggest_categorical('kernel_size', [1,3,5,7])
    num_encoder_layer = trial.suggest_categorical('num_encoder_layer', [1,3,5])
    nheads_ = trial.suggest_categorical('nheads', [1,4,8])
    # Add more hyperparameters here
    
    train_dataset = Dataset_Custom(root_path=root_path, flag='train', size=size, data_path=data_path, step_size = step_size_)
    val_dataset = Dataset_Custom(root_path=root_path, flag='val', size=size, data_path=data_path,step_size = step_size_)
    test_dataset = Dataset_Custom(root_path=root_path, flag='test', size=size, data_path=data_path,step_size = step_size_)
    
    # Create DataLoaders
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle_, drop_last = True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False,drop_last = True)
    test_loader = DataLoader(test_dataset, batch_size = batch_size, shuffle =False, drop_last = True)
    # input_channels, seq_length, mlp_hidden_size, output_channels, decompose_layers=3, wave='haar', mode='symmetric', nhead=8, d_model=None, num_encoder_layers=3, dropout=0.1, dilation = 2, kernel_size = 3)
    # Initialize the model
    model = DWT_MLP_Model(input_channels=321+4, seq_length=96, mlp_hidden_size=mlp_hidden_size, output_channels=321+4, decompose_layers = decompose_layer, mode= mode_, wave = wave_, kernel_size = kernel_size_, nhead = nheads_, dropout = dropout_rate, dilation = dilation_rate )
...
try:
    ...
    # Here, you can add any additional logging or cleanup code as needed
